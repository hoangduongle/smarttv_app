...
class _AbtractionScreenState extends State<AbtractionScreen> {
                  ...
                  child: Row(
                    children: [
                      controller.abtractions.value.isEmpty
                          ...
                          : _carouselSlider(controller, mapController),
                      controller.abtractions.value.isEmpty
                          ? const MapLoading()
                          : _mapMarker(controller, mapController),
                    ],
                  ),
                ),
              ],
  ...
  Expanded _carouselSlider(
      AbtractionController controller, SmartMapController mapController) {
    return Expanded(
        flex: 2,
        child: Padding(
              ...
              CarouselSlider.builder(
                ...
                itemBuilder: (context, index, realIndex) {
                  return AbtractionCard(
                      abtractionContent: controller.abtractions.value[index],
                      index: index,
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: FlutterMap(
                ...
                options: MapOptions(
                  keepAlive: true,
                  center: LatLng(10.838606359520535, 106.83161588307689),
                  zoom: 14.0,
                ),
                children: [
                  TileLayer(
                  ...
                  MarkerLayer(
                    markers: [
                      ...
                      for (int i = 0;
                          i < controller.abtractions.value.length;
                          i++)
                        Marker(
                          point: LatLng(
                            controller.abtractions.value[i].latidute ?? 0,
                            controller.abtractions.value[i].longtitude ?? 0,
                          ),
                          builder: (context) {
                            return SvgPicture.asset(
                              AppAssets.markerCircle,
                              color: controller.indexMarker == i
                                  ? AppColors.red
